...
function getAllEnvironments() {
	return new Promise(async (resolve, reject) => {
		try {
			let envObj = await ENV_DB.findAll();
			if (envObj.status !== responseStatus.success) resolve(false);
			resolve({ status: 'success', data: envObj.data });
		} catch (error) {
			resolve(false);
		}
	});
...
async function getNestedSuites(selectedSuiteId) {
	...
	suiteTraversalData[selectedSuiteId] = {
		id: selectedSuiteId,
		name: suiteName,
		path: [selectedSuiteId],
		depth: 0,
	};
